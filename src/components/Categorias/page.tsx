...
export default function Categorias({ selectedCategory, onCategoryChange }: CategoriaProps) {
    ...
    const hasPermissionSupervisao = session?.user.role === 'SUPERVISAO' || session?.user.role === 'ESTOQUE' || session?.user.role === 'COMPRAS' || session?.user.role === 'DIRETORIA' ; //esses perfis podem acessar as solicitacoes
    ...
    return (
        <>
            ...
            <div className={`h-20 w-20 flex flex-col justify-end items-center overflow-hidden relative rounded-md duration-300 group active:bg-light-blue ${hasPermissionSupervisao && 'hover:scale-110'}   `}>
                <div className={`${!hasPermissionSupervisao ? 'w-full h-full bg-[#f2f2f2]/80  absolute z-50' : 'hidden'}`} />
                <div className={`h-20 w-20  flex flex-col justify-end items-center rounded-md relative pb-1  group-hover:cursor-pointer active:bg-light-blue active:text-white ${hasPermissionSupervisao && filtroCategoria.name === 'Uniforme' && hasPermissionSupervisao || filtroCategoria.name === 'Uniforme' ? 'text-white bg-light-blue' : 'bg-light-gray text-gray-menu-icon'} `} onClick={() => handleFilter(categories.find(category=> category.name.includes("Uniforme")))}>
            ...
            <div className={`h-20 w-20 flex flex-col justify-end items-center overflow-hidden relative rounded-md duration-300 group active:bg-light-blue ${hasPermissionConsultoria && 'hover:scale-110'}  `}>
                <div className={`${!hasPermissionConsultoria ? 'w-full h-full bg-[#f2f2f2]/80  absolute z-50' : 'hidden'}`} />
                <div className={`h-20 w-20  flex flex-col justify-end items-center rounded-md relative pb-1  group-hover:cursor-pointer active:bg-light-blue active:text-white ${hasPermissionSupervisao && filtroCategoria.name === 'Cirúrgico'  && hasPermissionSupervisao || filtroCategoria.name === 'Cirúrgico'? 'text-white bg-light-blue' : 'bg-light-gray text-gray-menu-icon'}`} onClick={() => handleFilter(categories.find(category=> category.name.includes("Cirúrgico")))}>
            ...
            <div className={`h-20 w-20 flex flex-col justify-end items-center overflow-hidden relative rounded-md duration-300 group active:bg-light-blue ${hasPermissionSupervisao && 'hover:scale-110'}  `}>
                <div className={`${!hasPermissionSupervisao ? 'w-full h-full bg-[#f2f2f2]/80  absolute z-50' : 'hidden'}`} />
                <div className={`h-20 w-20  flex flex-col justify-end items-center rounded-md relative pb-1  group-hover:cursor-pointer active:bg-light-blue active:text-white ${hasPermissionSupervisao && filtroCategoria.name === 'Limpeza'  && hasPermissionSupervisao || filtroCategoria.name === 'Limpeza' ? 'text-white bg-light-blue' : 'bg-light-gray text-gray-menu-icon'}`} onClick={() => handleFilter(categories.find(category=> category.name.includes("Limpeza")))}>
            ...
            <div className={`h-20 w-20 flex flex-col justify-end items-center overflow-hidden relative rounded-md  duration-300 group active:bg-light-blue ${hasPermissionSupervisao && 'hover:scale-110'} `}>
                <div className={`${!hasPermissionSupervisao ? 'w-full h-full bg-[#f2f2f2]/80  absolute z-50' : 'hidden'}`} />
                <div className={`h-20 w-20  flex flex-col justify-end items-center rounded-md relative pb-1  group-hover:cursor-pointer active:bg-light-blue active:text-white ${hasPermissionSupervisao && filtroCategoria.name === 'Insumo'  && hasPermissionSupervisao || filtroCategoria.name === 'Insumo' ? 'text-white bg-light-blue' : 'bg-light-gray text-gray-menu-icon'} `} onClick={() => handleFilter(categories.find(category=> category.name.includes("Insumo")))}>
            ...
            <div className={`h-20 w-20 flex flex-col justify-end items-center overflow-hidden relative rounded-md duration-300 group active:bg-light-blue ${hasPermissionSupervisao && 'hover:scale-110'}  `}>
                <div className={`${!hasPermissionSupervisao ? 'w-full h-full bg-[#f2f2f2]/80  absolute z-50' : 'hidden'}`} />
                <div className={`h-20 w-20  flex flex-col justify-end items-center rounded-md relative pb-1  group-hover:cursor-pointer active:bg-light-blue active:text-white ${hasPermissionSupervisao && filtroCategoria.name === 'Escritório'  && hasPermissionSupervisao || filtroCategoria.name === 'Escritório' ? 'text-white bg-light-blue' : 'bg-light-gray text-gray-menu-icon'}`} onClick={() => handleFilter(categories.find(category=> category.name.includes("Escritório")))}>
